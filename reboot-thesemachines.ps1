...
Begin 
{
	...
	$RebootCommand = { shutdown /r /m $args[0] /t 5 /c "Down for maintenance" } 
}
Process 
{
	$Computers = @()
	...
	{
		write-host "Usage: reboot-thesemachines (get-content hosts.txt)"
		exit
	}
	...
	{
		Start-Job -Name $("Reboot" + $computer.Computer) -ScriptBlock $RebootCommand -ArgumentList $computer.Computer -ErrorAction Inquire
	}
